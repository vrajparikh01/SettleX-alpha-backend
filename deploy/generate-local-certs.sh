#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
#  SettleX Backend — Generate Local Self-Signed SSL Certificates
#
#  Usage (run from project root):
#    bash deploy/generate-local-certs.sh
#
#  Creates:
#    nginx/certs/private.key
#    nginx/certs/cert.pem
#
#  These are for LOCAL DEVELOPMENT ONLY.
#  Production certs come from GitHub Secrets (never committed to git).
# ─────────────────────────────────────────────────────────────────────────────

set -e

CERT_DIR="$(cd "$(dirname "$0")/.." && pwd)/nginx/certs"
mkdir -p "$CERT_DIR"

echo "Generating self-signed SSL certificate for local development..."
echo "Output directory: $CERT_DIR"
echo ""

openssl req -x509 \
  -newkey rsa:4096 \
  -nodes \
  -keyout "$CERT_DIR/private.key" \
  -out    "$CERT_DIR/cert.pem" \
  -days 365 \
  -subj "/C=IN/ST=Gujarat/L=Local/O=SettleX/CN=localhost" \
  -addext "subjectAltName=DNS:localhost,IP:127.0.0.1"

chmod 600 "$CERT_DIR/private.key"
chmod 644 "$CERT_DIR/cert.pem"

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  ✓ Certificates generated!"
echo "  Key : $CERT_DIR/private.key"
echo "  Cert: $CERT_DIR/cert.pem"
echo ""
echo "  These are gitignored — safe to commit the rest."
echo "  For local dev, start with:"
echo "    docker compose -f docker-compose.yml -f docker-compose.local.yml up --build"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
